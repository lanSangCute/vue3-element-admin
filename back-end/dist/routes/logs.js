...
router.post('/operation', async (req, res) => {
    try {
        ...
        const result = database_1.default.prepare('INSERT INTO operation_logs (userId, username, module, action, description, ip, userAgent, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(userId || null, username, module, action, description || null, ip || null, userAgent || null, now);
        res.json({
            code: 200,
            data: {
...
router.post('/login', async (req, res) => {
    try {
        ...
        const now = new Date().toISOString();
        const result = database_1.default.prepare('INSERT INTO login_logs (userId, username, ip, userAgent, status, createdAt) VALUES (?, ?, ?, ?, ?, ?)').run(userId || null, username, ip || null, userAgent || null, status, now);
        res.json({
            code: 200,
            data: {
                id: result.lastInsertRowid
